...
public class GameImpl implements Game {
    ...
    public void openChest() {
        if (map.getCell(player.getPosition()).hasChest() == false) {
            ...
            if (map.getCell(player.getPosition()).hasChest() == true && map.getCell(player.getPosition()).getChest().getNumItems() == 0) {
                System.out.println("The chest is empty.");
            } else {
                System.out.println("You collected these items: " + map.getCell(player.getPosition()).getChest().getItems());
