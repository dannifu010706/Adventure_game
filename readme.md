...
The "novice" portion of this assignment is to create three new classes that implement provided interfaces: `ItemImpl`
, `PositionImpl`, and `InventoryImpl`. Together, these classes will represent the basic building blocks for our
adventure game. In the "adept" and "jedi" sections, we will be *composing* and *aggregating* these objects together to
...
Create a `PlayerImpl` class which implements the `Player` interface. The `PlayerImpl` class should provide a single
...
Here, `width` and `height` indicate the dimensions of the 2D `Cell` grid, and should be non-negative (throw
...
Finally, the last class to create for this assignment is `GameImpl`, which implements the `Game` interface. A `Game` is
